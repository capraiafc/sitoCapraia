...
(() => {
  ...
  const reset = () => {
    ...
    submit.querySelector('[data-merch-submit-label]').textContent = 'Aggiungi prodotto'; renderGallery(); refreshUploadState();
  };
  ...
  const load = async () => { const { data, error } = await client().from('merch_products').select('id, name, price, description, image_url, image_path, available, published, created_at, merch_product_images(id, image_url, image_path, sort_order, is_primary)').order('created_at', { ascending: false }); if (error) throw error; products = data || []; render(); };
  const fill = (product) => { editingId = product.id; gallery = [...(product.merch_product_images || [])]; stagedFiles = []; removedImages = []; selectedPrimary = gallery.find((image) => image.is_primary) ? galleryKey(gallery.find((image) => image.is_primary)) : (gallery[0] ? galleryKey(gallery[0]) : null); form.elements.name.value = product.name; form.elements.price.value = Number(product.price).toFixed(2); form.elements.description.value = product.description || ''; form.elements.available.checked = product.available; form.elements.published.checked = product.published; submit.querySelector('[data-merch-submit-label]').textContent = 'Salva modifiche'; renderGallery(); modal.open(`Modifica prodotto: ${product.name}`); };
  const payload = () => { const amount = Number(form.elements.price.value); if (!Number.isFinite(amount) || amount < 0) throw new Error('Inserisci un prezzo valido.'); return { name: form.elements.name.value.trim(), price: Math.round((amount + Number.EPSILON) * 100) / 100, description: form.elements.description.value.trim() || null, available: form.elements.available.checked, published: form.elements.published.checked }; };
