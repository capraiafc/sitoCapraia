...
function featuredCard(match, next) {
  ...
  return `<${tag} class="${className}${played(match) ? ' results-feature-clickable' : ''}"${attributes}><div class="results-feature-top"><h3>${title}</h3><span class="results-venue-tag">${location(match)}</span></div>${competition(match)}<div class="results-feature-date">${dateMarkup(match)}</div><div class="results-scoreboard">${team(match, 'home')}<div class="results-score"><b>${score(match)}</b><small>${escapeHtml(statusLabels[match.status] || 'Da confermare')}</small>${penalties(match)}</div>${team(match, 'away')}</div><div class="results-feature-bottom"><span class="results-place">${escapeHtml(match.venue || 'Campo da confermare')}</span><span>${escapeHtml(match.phase || (match.match_day ? `Giornata ${match.match_day}` : ''))}</span></div></${tag}>`;
}
