...
serve(async (request) => {
  ...
  try {
    ...
    const emailResponse = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${resendKey}`, 'Content-Type': 'application/json', 'User-Agent': 'capraiafc-member-renewal/1.0', 'Idempotency-Key': `member-renewal/${requestId}` }, body: JSON.stringify({
      ...
      html: `<h1>Grazie per aver scelto Capraia FC</h1><p>Ciao <strong>${escapeHtml(name)}</strong>, il tuo tesseramento per la stagione <strong>${escapeHtml(season)}</strong> è stato rinnovato con successo.</p><table role="presentation" cellpadding="8" style="border-collapse:collapse"><tbody><tr><th align="left">Numero tessera</th><td>${escapeHtml(cardNumber)}</td></tr><tr><th align="left">Socio dal</th><td>${escapeHtml(memberSince)}</td></tr><tr><th align="left">Metodo di pagamento</th><td>${escapeHtml(member.payment_method)}</td></tr><tr><th align="left">Totale rinnovo</th><td>${escapeHtml(amountLabel)}</td></tr></tbody></table><h2>Fac-simile della tua tessera</h2><p><img src="${escapeHtml(frontCard)}" alt="Fronte tessera Capraia FC" width="500" style="max-width:100%;height:auto;display:block" /></p><p><img src="${escapeHtml(backCard)}" alt="Retro tessera Capraia FC" width="500" style="max-width:100%;height:auto;display:block" /></p><p><strong>Intestatario:</strong> ${escapeHtml(name)} · <strong>Tessera n.</strong> ${escapeHtml(cardNumber)} · <strong>Socio dal</strong> ${escapeHtml(memberSince)}</p><p>Ti aspettiamo al campo. Forza Capraia!</p>`,
    }) });
